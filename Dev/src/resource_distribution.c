...
void distribute_resources_by_geography(AREA_DATA *area)
{
    ...
    {
        char log_buf[256];
    sprintf(log_buf, "RESOURCES: Distributing resources for %s (features: %d)", (char *)area->name, geo_features);
    log_string(log_buf);
    }
...
void analyze_resource_needs(AREA_DATA *area)
{
    ...
    if (!has_food)
    {
        {
            char log_buf[256];
        sprintf(log_buf, "RESOURCES: %s needs food imports", (char *)area->name);
    log_string(log_buf);
        }
    ...
    if (!has_building_materials)
    {
        {
            char log_buf[256];
        sprintf(log_buf, "RESOURCES: %s needs building materials", (char *)area->name);
    log_string(log_buf);
        }
    }
}
