...
void record_player_action(CHAR_DATA *player, int action_type, void *target, AREA_DATA *area)
{
    ...
    switch (action_type)
    {
        case PLAYER_ACTION_KILL_MOB:
            ...
            sprintf(buf, "Killed %s", action->target_mob->short_descr);
...
void player_killed_mob(CHAR_DATA *player, CHAR_DATA *mob)
{
    ...
    record_kill_event(player->name, mob->short_descr, "combat",
                     mob->in_room ? mob->in_room->area->name : "unknown");
}
...
void complete_gradual_change(GRADUAL_CHANGE *change)
{
    ...
    /* Announce completion */
    smart_announce(
        change->completion_effect ? change->completion_effect : "Area evolution complete",
    ...
    /* Record in history */
    record_area_creation("Beeler", change->area->name, change->change_description);
}
