...
void mob_write_book(CHAR_DATA *author, int book_type, char *subject)
{
    ...
    {
        char log_buf[256];
    sprintf(log_buf, "BOOK WRITING: %s is writing about '%s'", (char *)author->short_descr, subject);
    log_string(log_buf);
    }

    /* Generate content */
    content = generate_book_content(book_type, subject, (char *)author->short_descr, "scribe");
    if (!content)
    {
    ...
    /* Create book object */
    book_obj = create_book_object(title, (char *)author->short_descr, content, &vnum);
    if (!book_obj)
    {
    ...
    book->author = str_dup((char *)author->short_descr);
    ...
    record_book_written((char *)author->short_descr, title, vnum, content);
    ...
    if (book_type == BOOK_TYPE_HISTORY || total_books_written % 10 == 0)
    {
        smart_announce(
            title,
            (char *)"A new work has been published",
            EVENT_CATEGORY_CULTURE,
            ANNOUNCE_PRIORITY_LOW,
            (char *)(author->in_room ? author->in_room->area->name : "Unknown")
        );
    }

    {
        char log_buf[256];
    sprintf(log_buf, "BOOK WRITING: Completed! '%s' by %s (vnum %d)", title, (char *)author->short_descr, vnum);
    log_string(log_buf);
    }
}
...
void queue_book_writing(CHAR_DATA *author, int book_type, char *subject)
{
    ...
    sprintf(log_buf, "BOOK WRITING: Queued task for %s to write about '%s'",
            (char *)author->short_descr, subject);
    log_string(log_buf);
    }
}
