...
int get_mob_power_level(CHAR_DATA *mob)
{
    ...
    if (strstr((char *)mob->short_descr, "king") || strstr((char *)mob->short_descr, "King"))
        return POWER_KING;
    if (strstr((char *)mob->short_descr, "lord") || strstr((char *)mob->short_descr, "noble"))
        return POWER_NOBLE;
    if (strstr((char *)mob->short_descr, "guild master") || strstr((char *)mob->short_descr, "guildmaster"))
        return POWER_GUILD_MASTER;
    if (strstr((char *)mob->short_descr, "merchant") || strstr((char *)mob->short_descr, "trader"))
        return POWER_MERCHANT;
    if (strstr((char *)mob->short_descr, "blacksmith") || strstr((char *)mob->short_descr, "craftsman"))
...
bool mob_creation_rate_limited(CHAR_DATA *mob)
{
    ...
    if (limit->creations_this_week >= max_per_week)
    {
        {
            char log_buf[256];
        sprintf(log_buf, "MOB CREATION: Rate limit - %s has created %d/%d this week", (char *)mob->short_descr, limit->creations_this_week, max_per_week);
    log_string(log_buf);
        }
        return TRUE;
    }
...
CHAR_DATA *mob_create_npc(CHAR_DATA *creator, CREATION_TYPE npc_type, char *reason)
{
    ...
    if (!can_mob_create(creator, npc_type))
    {
        {
            char log_buf[256];
        sprintf(log_buf, "MOB CREATION: %s lacks power to create type %d", creator(char *)->short_descr, npc_type);
    log_string(log_buf);
        }
        return NULL;
    }
    ...
    {
        case CREATION_APPRENTICE:
            sprintf(short_desc, "an apprentice of %s", creator(char *)->short_descr);
    ...
    pMobIndex(char *)->short_descr = str_dup(short_desc);
    ...
    record_mob_creation(creator(char *)->short_descr, new_(char *)mob->short_descr, reason);

    {
        char log_buf[256];
    sprintf(log_buf, "MOB CREATION: %s created %s (vnum %d) - %s", creator(char *)->short_descr, new_(char *)mob->short_descr, next_mob_vnum, reason);
    log_string(log_buf);
    }
...
OBJ_DATA *mob_create_object(CHAR_DATA *creator, int obj_type, char *name, char *description)
{
    ...
    pObjIndex(char *)->short_descr = str_dup(name);
    ...
    {
        char log_buf[256];
    sprintf(log_buf, "MOB CREATION: %s created object '%s' (vnum %d)", creator(char *)->short_descr, name, next_obj_vnum);
    log_string(log_buf);
    }
        ...
        for (mob = first_char; mob; mob = mob->next)
        {
                ...
                get_mob_power_level(mob) >= POWER_NOBLE)
            {
                if (!mob_creation_rate_limited(mob))
                {
                    ...
                    {
                        char log_buf[256];
                    sprintf(log_buf, "AUTO CREATION: %s hired guard due to low safety", (char *)mob->short_descr);
    log_string(log_buf);
                    }
        ...
        for (mob = first_char; mob; mob = mob->next)
        {
                ...
                get_mob_power_level(mob) >= POWER_MERCHANT)
            {
                if (!mob_creation_rate_limited(mob))
                {
                    ...
                    {
                        char log_buf[256];
                    sprintf(log_buf, "AUTO CREATION: %s hired worker due to low population", (char *)mob->short_descr);
    log_string(log_buf);
                    }
                    break;
                }
    ...
    for (record = first_creation; record && count < 20; record = record->next)
    {
        if (record->created_mob)
        {
            sprintf(buf, "%s created %s - %s\n\r",
                    record->creator(char *)->short_descr,
                    record->created_(char *)mob->short_descr,
                    record->reason);
        }
        else if (record->created_obj)
        {
            sprintf(buf, "%s created %s\n\r",
                    record->creator(char *)->short_descr,
                    record->created_obj(char *)->short_descr);
        }
        send_to_char(buf, ch);
        count++;
    }
