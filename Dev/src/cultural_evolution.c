...
AREA_CULTURE *create_area_culture(AREA_DATA *area)
{
    ...
    sprintf(log_buf, "CULTURAL EVOLUTION: Created culture for %s: %s",
            (char *)area->name, culture->cultural_name);
    log_string(log_buf);
    }
...
void evolve_culture(AREA_CULTURE *culture)
{
    ...
    {
        char log_buf[256];
    sprintf(log_buf, "CULTURAL EVOLUTION: Evolving culture for %s", culture->(char *)area->name);
    log_string(log_buf);
    }
